...
void SparkControlLoop() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (v == 1) {
      sparkx_switch = i;
      if (i < 2) 
        sparkcontrol_switch |= (v << i);
      else
        sparkcontrol_switch |= (v << (i+1));          
    }
  }
#endif
  ...
  if (sparkx_switch != last_sparkx_switch && sparkx_switch >= 0) {
    swx_dat[4] = sparkx_switch;
    pcX2->setValue(swx_dat, sizeof(swx_dat));
    pcX2->notify();
    last_sparkx_switch = sparkx_switch;
    Serial.print("Switched ");
    Serial.println(sparkx_switch);
  }

  if (sparkcontrol_switch != last_sparkcontrol_switch) {
    sw_dat[0] = sparkcontrol_switch;
    pcData1->setValue(sw_dat, 1);
    pcData1->notify();
    last_sparkcontrol_switch = sparkcontrol_switch;
    Serial.print("Switched ");
    Serial.println(sparkcontrol_switch);
  }  
