...
class BLEMidiCentral:
    ...
    def _reset(self):
        ...
        self._characteristics_found = {}
        self._descriptors_found = {}
        self._to_set_notify = []
    ...
    def _irq(self, event, data):
        if event == _IRQ_SCAN_RESULT:
            ...
            self._services_found[start_handle] = (uuid, conn_handle, end_handle)
            
            if not self._start_handle or self._start_handle > start_handle:
                self._start_handle = start_handle
            if not self._end_handle or self._end_handle < end_handle:    
            ...
            self._characteristics_found[def_handle] = (uuid, conn_handle, value_handle, properties)
            ...
            self._descriptors_found[dsc_handle] = (uuid, conn_handle)
            
            if uuid == bluetooth.UUID(0x2902):
                self._to_set_notify.append(dsc_handle)
            ...
            if conn_handle == self._conn_handle and value_handle == self._rx_value_handle:
                ...
                byt = bytes(char_data)
                print("Received {} {} {}".format(value_handle, byt.hex(), byt))
...
def demo(central):
    ...
    for d in central._to_set_notify:
        central._ble.gattc_write(central._conn_handle, d, b'\x01\x00', 0)
        time.sleep_ms(200)
        print("Registered for notify on handle", d)


    for c in central._characteristics_found:
        if central._characteristics_found[c][3] & 0x02:
            central._ble.gattc_read(central._conn_handle, c)
            time.sleep_ms(200)
            #print("Read requested", c)
